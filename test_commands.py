...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup, KeyboardButton, ReplyKeyboardRemove, BotCommand, MenuButtonCommands, MenuButtonWebApp, MenuButtonDefault
...
# Счетчик для тестирования удаления сообщений после нажатия кнопки
# Формат: {chat_id: counter}
click_counters = {}
...
async def help(message: types.Message):
    await message.answer(
        ...
        "/test_delete_on_click - протестировать удаление сообщения после нажатия кнопки\n"
        "/setup_menu_button - настроить меню бота (кнопка рядом с вложениями)\n"
        "/setup_bot_menu_button - настроить кнопку меню рядом с эмодзи (Bot Menu Button)"
    )
...
# ============================================================
# ТЕСТИРОВАНИЕ: УДАЛЕНИЕ СООБЩЕНИЯ ПОСЛЕ НАЖАТИЯ КНОПКИ
# ============================================================
@dp.message(Command('test_delete_on_click'))
async def test_delete_on_click(message: types.Message):
    """Тестовая команда для проверки удаления сообщения после нажатия кнопки"""
    if message.chat.type not in ['group', 'supergroup']:
        await message.answer("❌ Эта команда работает только в группах!")
        return
    
    # Инициализируем счетчик для группы, если его нет
    if message.chat.id not in click_counters:
        click_counters[message.chat.id] = 0
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Нажми меня (сообщение удалится)",
                    callback_data="btn_delete_me"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📊 Показать счетчик",
                    callback_data="btn_show_counter"
                )
            ]
        ]
    )
    ...
    await message.answer(
        "🧪 Тест удаления сообщения после нажатия кнопки\n\n"
        "💡 Идея: нажми на кнопку → действие выполнится → сообщение удалится\n"
        "Это поможет не засорять чат!\n\n"
        f"📊 Текущий счетчик: {click_counters[message.chat.id]}",
        reply_markup=keyboard
    )
    logger.info(f"✅ Тест удаления сообщения отправлен в группу {message.chat.title}")

# Обработчик нажатия на кнопку "Удалить меня"
@dp.callback_query(F.data == "btn_delete_me")
async def handle_delete_on_click(callback: types.CallbackQuery):
    """Обработчик нажатия на кнопку - увеличивает счетчик и удаляет сообщение"""
    chat_id = callback.message.chat.id
    
    # Увеличиваем счетчик
    if chat_id not in click_counters:
        click_counters[chat_id] = 0
    click_counters[chat_id] += 1
    
    # Показываем уведомление
    await callback.answer(
        f"✅ Счетчик увеличен! Теперь: {click_counters[chat_id]}",
        show_alert=False
    )
    ...
    try:
        ...
        logger.info(
            f"✅ Сообщение {callback.message.message_id} удалено после нажатия кнопки "
            f"в группе {chat_id}. Счетчик: {click_counters[chat_id]}"
        )
    except Exception as e:
        error_msg = str(e).lower()
        if "not enough rights" in error_msg or "chat admin" in error_msg:
            # Если нет прав, просто редактируем сообщение
            await callback.message.edit_text(
                "❌ Бот не может удалить сообщение (нет прав администратора)\n"
                f"✅ Но действие выполнено! Счетчик: {click_counters[chat_id]}"
            )
            logger.warning(f"⚠️ Не удалось удалить сообщение: нет прав администратора")
        elif "message to delete not found" in error_msg:
            logger.warning(f"⚠️ Сообщение уже было удалено")
        else:
            # Другие ошибки - редактируем сообщение
            await callback.message.edit_text(
...
# Обработчик показа счетчика
@dp.callback_query(F.data == "btn_show_counter")
async def handle_show_counter(callback: types.CallbackQuery):
    """Обработчик показа счетчика без удаления сообщения"""
    chat_id = callback.message.chat.id
    
    if chat_id not in click_counters:
        click_counters[chat_id] = 0
    
    # Просто показываем уведомление, не редактируем сообщение
    await callback.answer(
        f"📊 Текущий счетчик: {click_counters[chat_id]}",
        show_alert=False
    )
    
    logger.info(f"✅ Пользователь {callback.from_user.id} запросил счетчик: {click_counters[chat_id]}")

# ============================================================
# ТЕСТИРОВАНИЕ: МЕНЮ БОТА (КНОПКА РЯДОМ С ВЛОЖЕНИЯМИ)
# ============================================================
@dp.message(Command('setup_menu_button'))
async def setup_menu_button(message: types.Message):
    """Настройка меню бота (кнопка рядом с вложениями)"""
    # Создаем список команд для меню
    commands = [
        BotCommand(command="start", description="🚀 Начать работу с ботом"),
        BotCommand(command="help", description="📖 Показать справку"),
        BotCommand(command="test_group", description="🧪 Тест отправки в группе"),
        BotCommand(command="test_inline_buttons", description="🔘 Тест inline кнопок"),
        BotCommand(command="test_reply_keyboard", description="⌨️ Тест Reply клавиатуры"),
        BotCommand(command="test_delete_on_click", description="🗑️ Тест удаления после клика"),
    ]
    ...
    try:
        ...
        await message.answer(
            "✅ Меню бота настроено!\n\n"
            "💡 Теперь рядом с кнопкой вложений (📎) появится кнопка меню бота\n"
            "При нажатии на неё откроется меню с командами:\n\n"
            "• /start - Начать работу\n"
            "• /help - Справка\n"
            "• /test_group - Тест группы\n"
            "• /test_inline_buttons - Тест inline кнопок\n"
            "• /test_reply_keyboard - Тест Reply клавиатуры\n"
            "• /test_delete_on_click - Тест удаления\n\n"
            "🔍 Попробуй нажать на кнопку рядом с вложениями!"
        )
        logger.info(f"✅ Меню бота настроено пользователем {message.from_user.id}")
    except Exception as e:
        await message.answer(
            f"❌ Ошибка настройки меню: {str(e)}"
        )
        logger.error(f"❌ Ошибка настройки меню бота: {e}")

@dp.message(Command('setup_bot_menu_button'))
async def setup_bot_menu_button(message: types.Message):
    """Настройка Bot Menu Button (кнопка рядом с эмодзи)"""
    try:
        # MenuButtonCommands - показывает команды бота
        # Это то, что ты видел - кнопка рядом с эмодзи, при нажатии открывает меню с командами
        menu_button = MenuButtonCommands()
        
        # Устанавливаем кнопку меню глобально (для всех чатов)
        # Если указать chat_id=None, устанавливается глобально
        # Для групп может не работать, поэтому устанавливаем глобально
        await bot.set_chat_menu_button(
            chat_id=None,  # Глобально для всех чатов
            menu_button=menu_button
        )
        
        await message.answer(
            "✅ Bot Menu Button настроена глобально!\n\n"
            "💡 Теперь рядом с кнопкой эмодзи (😀) появится кнопка меню бота\n"
            "При нажатии на неё откроется меню с командами бота\n\n"
            "🔍 Попробуй нажать на кнопку рядом с эмодзи в любом чате с ботом!\n\n"
            "📝 Разница между типами меню:\n"
            "• Bot Commands Menu (/setup_menu_button) - кнопка рядом с вложениями (📎)\n"
            "• Bot Menu Button (эта команда) - кнопка рядом с эмодзи (😀)\n\n"
            "💡 Оба меню показывают команды, но расположены в разных местах!"
        )
        logger.info(f"✅ Bot Menu Button настроена глобально пользователем {message.from_user.id}")
    except Exception as e:
        error_msg = str(e).lower()
        if "not implemented" in error_msg or "not supported" in error_msg:
            await message.answer(
                "❌ Эта функция может быть недоступна в текущей версии Telegram\n\n"
                "💡 Попробуй обновить Telegram до последней версии\n"
                "Или используй /setup_menu_button для настройки меню рядом с вложениями"
            )
        elif "invalid chat_id" in error_msg or "chat_id" in error_msg:
            # Пробуем установить для личного чата
            if message.chat.type == 'private':
                try:
                    ...
                    await message.answer(
                        "✅ Bot Menu Button настроена для личного чата!\n\n"
                        "💡 Теперь рядом с кнопкой эмодзи (😀) появится кнопка меню бота\n"
                        "При нажатии на неё откроется меню с командами бота\n\n"
                        "🔍 Попробуй нажать на кнопку рядом с эмодзи!"
                    )
                    logger.info(f"✅ Bot Menu Button настроена для личного чата {message.chat.id}")
                except Exception as e2:
                    await message.answer(
                        f"❌ Ошибка настройки Bot Menu Button: {str(e2)}\n\n"
                        f"💡 Эта функция может работать только в личных чатах\n"
                        f"Или используй /setup_menu_button для альтернативного меню"
                    )
                    logger.error(f"❌ Ошибка настройки Bot Menu Button для личного чата: {e2}")
            else:
                await message.answer(
                    "❌ Bot Menu Button можно настроить только в личных чатах!\n\n"
                    "💡 Напиши боту в личном чате (не в группе) команду /setup_bot_menu_button\n"
                    "Или используй /setup_menu_button для настройки меню рядом с вложениями (работает везде)"
                )
        else:
            await message.answer(
                f"❌ Ошибка настройки Bot Menu Button: {str(e)}\n\n"
                f"💡 Попробуй использовать /setup_menu_button для альтернативного меню"
            )
        logger.error(f"❌ Ошибка настройки Bot Menu Button: {e}")
...
async def main():
    ...
    # Настраиваем меню бота при запуске
    commands = [
        BotCommand(command="start", description="🚀 Начать работу с ботом"),
        BotCommand(command="help", description="📖 Показать справку"),
        BotCommand(command="test_group", description="🧪 Тест отправки в группе"),
